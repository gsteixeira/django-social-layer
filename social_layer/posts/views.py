...
def new_post(request):
    ...
    if request.method == "POST":
        form = PostForm(request.POST, request.FILES, initial={"owner": request.user})
        if form.is_valid():
            post = form.save(commit=False)
            post.owner = get_social_profile(request)
            post.save()
            if form.allow_media and len(request.FILES) > 0:
                media = handle_upload_file(
                    file_post=request.FILES.get("media"),
                    quality=1,
                    Model=PostMedia,
    ...
    return render(
        request,
        "social_layer/posts/new_post.html",
        {
            "form": form,
        },
    )
