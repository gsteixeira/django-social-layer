...
def setup_profile(request):
    ...
    if request.method == "POST":
        ...
        for upload_file in request.FILES:
            foto = handle_upload_file(
                file_post=request.FILES[upload_file],
                quality=2,
...
def view_profile(request, pk):
    ...
    if alt_view:
        ...
        if response:
            return response
    data = {"sprofile": sprofile, "comments": Comment.objects.filter(author=sprofile)}
...
def list_profiles(request):
    ...
    data = {
        "list_profiles": list_profiles[0:100],
    }
